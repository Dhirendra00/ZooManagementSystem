...
if (isset($_POST['add_reptile'])) {
    $target ="photo/reptile/".basename($_FILES['image']['name']); 
    move_uploaded_file($_FILES['image']['tmp_name'],$target);
     $target1="photo/reptile/sponsor/".basename($_FILES['sponsor']['name']);
    move_uploaded_file($_FILES['sponsor']['tmp_name'],$target1);
  // $filename = $_FILES["image"]["name"];
  // $tempname = $_FILES["image"]["tmp_name"];
  // $location = "photo/".$filename;
  // move_uploaded_file($tempname, $location);
$stmt=$pdo->prepare("INSERT INTO tbl_reptile(species_name,given_name,category,av_life_span,date_of_birth,date_joined,dietry_requirement,natural_habitat,location_code,dimension_in_zoo,average_dimension,av_no_of_offspring,reproduction_type,gender,image,sponsor) VALUES (:species_name,:given_name,:category,:av_life_span,:date_of_birth,:date_joined,:dietry_requirement,:natural_habitat,:location_code,:dimension_in_zoo,:average_dimension,:av_no_of_offspring,:reproduction_type,:gender,:image,:sponsor)");
$criteria =[
...
'av_no_of_offspring'=>$_POST['av_no_of_offspring'],
...
<!-- <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 " style="background-color: rgb(23,22,22); opacity: 0.5; border-radius: 5rem;"  > -->
          ...
          <div class="row">
            <h1 class="text-danger text-center ml-3 ">Add Reptile</h1> <a href="admin_zoo.php"><button class="btn btn-outline-danger my-3 my-sm-0 text-capitalize" style="background-color: rgb(50,187,179); width: 10rem; font-size: 20px; font-weight: bold; margin-left: 37rem;" type="submit">Back</button></a> <br>
...
<!-- fourth row -->
  ...
  <div class="form-group col-md-4">
      <label>Average no. of offspring</label>
      <input type="text" class="form-control" name="av_no_of_offspring">
    </div>
  
 <!-- fifth row -->
     <div class="form-group col-md-4">
      <label>Reproduction Type</label>
      <input type="text" class="form-control" name="reproduction_type">
    </div>
...
<div class="form-group col-md-4">
  ...
  <div class="form-group row p-2">
    <div class="col-sm-6">
      <button type="submit" name="add_reptile" class="btn" style="background-color: rgb(50,187,179); font-size: 20px; font-weight: bold; ">Confirm</button>
