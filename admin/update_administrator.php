...
if (isset($_POST['update_administrator'])) {
  ...
  $update=$pdo->prepare("UPDATE tbl_administrator SET first_name=:first_name,last_name=:last_name,contact=:contact,address=:address,email=:email,bio=:bio,image=:image WHERE id=:id");
  $criteria =[
  ...
  if ($update->execute($criteria)) {
    ?>
     <strong>updated successfully.Pree Ok!! <button> <a href="admin_administrator.php">OK</a></button></strong>
     <?php
}
else{
  echo "error";
...
<!-- <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 " style="background-color: rgb(23,22,22); opacity: 0.5; border-radius: 5rem;"  > -->
  ...
  <div class="form-group row p-2">
    <div class="col-sm-6">
      <button type="submit" name="update_administrator" class="btn ml-0" style="background-color: rgb(50,187,179); width: 10rem; font-size: 20px; font-weight: bold; margin-left: 47.5rem;">Confirm</button>
