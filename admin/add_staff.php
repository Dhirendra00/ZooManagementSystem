...
if (isset($_POST['signup'])) {
  $store=$pdo->prepare("INSERT INTO tbl_user(first_name,last_name,address,username,password,contact,email,user_type) VALUES(:first_name,:last_name,:address,:username,:password,:contact,:email,'admin')");
  $criteria =[
  ...
  if ($store->execute($criteria)) {
    ?>
     <strong>Added successfully.Pree Ok!! <button> <a href="admin_staff.php">OK</a></button></strong>
     <?php
}
else{
  echo "error";
...
<!-- <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 " style="background-color: rgb(23,22,22); opacity: 0.5; border-radius: 5rem;"  > -->
          ...
          <div class="row">
            <h1 class="text-danger text-center ml-3 ">Add Staff</h1> <a href="admin_staff.php"><button class="btn btn-outline-danger my-3 my-sm-0 text-capitalize" style="background-color: rgb(50,187,179); width: 10rem; font-size: 20px; font-weight: bold; margin-left: 47.5rem;" type="submit">Back</button></a> <br>
  ...
  <div class="form-group row p-2">
    <div class="col-sm-6">
      <button type="submit" name="signup" class="btn ml-0" style="background-color: rgb(50,187,179); width: 10rem; font-size: 20px; font-weight: bold; margin-left: 47.5rem;">Confirm</button>
