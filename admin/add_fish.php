...
if (isset($_POST['add_fish'])) {
    ...
    move_uploaded_file($_FILES['image']['tmp_name'],$target);
     $target1="photo/amphibian/sponsor/".basename($_FILES['sponsor']['name']);
    move_uploaded_file($_FILES['sponsor']['tmp_name'],$target1);
  // $filename = $_FILES["image"]["name"];
  // $tempname = $_FILES["image"]["tmp_name"];
  // $location = "photo/".$filename;
  // move_uploaded_file($tempname, $location);
$stmt=$pdo->prepare("INSERT INTO tbl_fish(species_name,given_name,category,av_life_span,date_of_birth,date_joined,dietry_requirement,natural_habitat,location_code,dimension_in_zoo,average_dimension,av_body_temperature,water_type,colour_variant,gender,image,sponsor) VALUES (:species_name,:given_name,:category,:av_life_span,:date_of_birth,:date_joined,:dietry_requirement,:natural_habitat,:location_code,:dimension_in_zoo,:average_dimension,:av_body_temperature,:water_type,:colour_variant,:gender,:image,:sponsor)");
$criteria =[
