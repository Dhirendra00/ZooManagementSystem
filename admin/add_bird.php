...
if (isset($_POST['add_bird'])) {
    ...
    $target1="photo/bird/sponsor/".basename($_FILES['sponsor']['name']);
    move_uploaded_file($_FILES['sponsor']['tmp_name'],$target1);
  // $filename = $_FILES["image"]["name"];
  // $tempname = $_FILES["image"]["tmp_name"];
  // $location = "photo/".$filename;
  // move_uploaded_file($tempname, $location);
$stmt=$pdo->prepare("INSERT INTO tbl_bird(species_name,given_name,category,av_life_span,date_of_birth,date_joined,dietry_requirement,natural_habitat,location_code,dimension_in_zoo,average_dimension,clutch_size,nest_construction_method,wingspan,colour_variant,gender,image,fly,sponsor) VALUES (:species_name,:given_name,:category,:av_life_span,:date_of_birth,:date_joined,:dietry_requirement,:natural_habitat,:location_code,:dimension_in_zoo,:average_dimension,:clutch_size,:nest_construction_method,:wingspan,:colour_variant,:gender,:image,:fly,:sponsor)");
$criteria =[
